(() => {
  ...
  const googleEmbedUrl = value => {
    ...
    try {
      ...
      const parts = url.pathname.split('/').filter(Boolean);
      if (host === 'drive.google.com' && id) return `https://drive.google.com/file/d/${id}/preview`;
  ...
  const mountLivePreview = frame => {
    if (!frame || frame.dataset.liveMounted === 'true') return;
    ...
    requestAnimationFrame(() => frame.classList.add('has-live'));
    liveObserver?.unobserve(frame);
  };

  const observeForLivePreview = frame => {
    if (!frame) return;
    if (!('IntersectionObserver' in window)) return;
    if (!liveObserver) {
      liveObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting && entry.intersectionRatio >= 0.12) mountLivePreview(entry.target);
        });
      }, { root:null, rootMargin:'80px 0px 80px 0px', threshold:[0,.12,.35] });
    }
    liveObserver.observe(frame);
  };
  ...
  const makePreview = (embedUrl, sourceUrl, title = '') => {
    ...
    thumbnail?.addEventListener('error', () => { thumbnail.remove(); frame.classList.remove('has-thumbnail'); });
    frame.querySelector('.smart-media-fullscreen')?.addEventListener('click', event => {
      event.preventDefault(); event.stopPropagation();
      openViewer(frame.dataset.embedUrl, frame.dataset.title);
    });
    observeForLivePreview(frame);
    return frame;
  };
  ...
  style.textContent = `
    ...
    .smart-media-fullscreen{position:absolute;right:12px;bottom:12px;z-index:5;display:inline-flex;align-items:center;gap:8px;padding:8px 11px;border:1px solid rgba(255,255,255,.2);border-radius:999px;background:rgba(8,10,14,.82);color:#fff;font:600 12px/1.1 DM Sans,sans-serif;backdrop-filter:blur(10px);cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.28)}
    .smart-media-fullscreen b{font-size:16px;line-height:1}
    .smart-media-overlay{position:fixed;inset:0;z-index:99999;display:none;background:#050608}.smart-media-overlay.open{display:block}
    .smart-media-overlay-shell{position:absolute;inset:0;background:#050608}.smart-media-overlay-content{position:absolute;inset:0}.smart-media-overlay-content iframe{display:block;width:100%;height:100%;border:0;background:#fff}
    ...
    @media(max-width:700px){.smart-media-fullscreen span{display:none}.smart-media-fullscreen{width:40px;height:40px;justify-content:center;padding:0}.smart-media-preview-inner{left:8px;bottom:8px;max-width:calc(100% - 64px);padding:8px}.smart-media-preview-icon{width:44px;height:44px;flex-basis:44px}.smart-media-preview-copy small{display:none}}
  `;
