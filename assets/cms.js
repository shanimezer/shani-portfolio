(() => {
  ...
  window.PortfolioCMS = {
    ...
    renderGrid(container, options={}){
      ...
      container.innerHTML=list.length ? list.map(p=>`<a class="card cms-card" data-cat="${this.escape(p.category)}" href="${this.projectUrl(p, options.prefix||'../')}"><div class="cms-cover" style="--project-accent:${this.escape(p.accent||'#b9bec8')};background-image:url('${this.escape(p.cover||'')}')"></div><div class="card-body"><div class="meta"><span>${this.escape(p.categoryLabel||this.categories[p.category]||p.category)}</span><span>${this.escape(p.year||'')}</span></div><h3>${this.escape(p.title)}</h3><p class="muted">${this.escape(p.summary||'')}</p></div></a>`).join('') : `<div class="empty-state"><h3>No projects here yet.</h3><p>Add one from the Admin panel.</p></div>`;
    }
  };
